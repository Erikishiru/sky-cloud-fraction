...
# Results directory
resultsDir = './results_sky/' + datasetName + str(IMAGE_SIZE) + '/'
...
modelSavePath = './models_sky/final_' + datasetName + str(IMAGE_SIZE)
ckptSavePath = './models_sky/bestCKPT_' + datasetName + str(IMAGE_SIZE)
...
# Extract Cloud Masks
images_cm = deepcopy(images)
images_cm[:,:,:,0][gtMaps==0] = 0
images_cm[:,:,:,1][gtMaps==0] = 0
images_cm[:,:,:,2][gtMaps==0] = 0

im = Image.fromarray(gtMaps[0].astype(np.uint8))
im.save("testGT.png")
im = Image.fromarray(images_cm[0].astype(np.uint8))
im.save("testCM.png")
