...
def train(
    ...
    for epoch in range(max_epochs):
        ...
        for images, labels, img_fname in tqdm(train_dataloader):
            images = images.to(device)
            labels = labels.to(device)
            # print("Input and Target Tensors", images.shape, labels.shape)
            # print("Target range", labels.min(), labels.max())
        ...
        if val_dataloader:
            ...
            with torch.no_grad():
                for images, labels, img_fname in tqdm(val_dataloader):
                    images = images.to(device)
                    labels = labels.to(device)
    ...
    save_loss_plot(train_losses, val_losses, save_path=experiments_dir)
    logger.info("Finished Training.\n")
...
def main(cfg):
    ...
    save_log(save_path=f"{str(experiments_dir)}/{cfg.experiment_name}.log")
